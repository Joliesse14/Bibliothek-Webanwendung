...
public class AdminServlet extends HttpServlet {
  ...
  protected void doDelete(HttpServletRequest req, HttpServletResponse res)
    ...
    if ("/buecher".equals(req.getPathInfo())) {
      String isbn = req.getParameter("isbn");
      try (Connection con = ds.getConnection()) {
        // Schritt 1 — Aktive Ausleihen prüfen
        ...
        rsCheck.next();
        long aktiveAusleihen = rsCheck.getLong(1);
        if (aktiveAusleihen > 0) {
          // Schritt 2 — Aktive Ausleihen automatisch abschließen
          PreparedStatement psClose = con.prepareStatement(
              "UPDATE ausleihe SET zurueckgegeben_am = NOW() " +
              "WHERE buchexemplar_id IN " +
              "(SELECT id FROM buchexemplar WHERE isbn = ?) " +
              "AND zurueckgegeben_am IS NULL");
          psClose.setString(1, isbn);
          psClose.executeUpdate();
        }
        // Schritt 3 — Buch löschen
        PreparedStatement ps = con.prepareStatement(
            "DELETE FROM buch WHERE isbn = ?");
        ps.setString(1, isbn);
        ps.executeUpdate();
        res.getWriter().write("✓ Buch entfernt. " +
            (aktiveAusleihen > 0 ? aktiveAusleihen + " aktive Ausleihe(n) wurden abgeschlossen." : ""));
      } catch (SQLException e) { throw new ServletException(e); }
    }
  }
